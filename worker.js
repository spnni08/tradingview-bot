export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST" && url.pathname === "/checklist") {
      const cklSecret = url.searchParams.get("secret");
      if (cklSecret !== env.WEBHOOK_SECRET) return Response.json({ error: "Unauthorized" }, { status: 401 });
      let body;
    ...
    if (request.method === "POST" && (url.pathname === "/webhook" || url.pathname === "/backtest")) {
      ...
      try {
        ...
        ruleScore = { score: 0, notes: [] };
      }

      // ── Credit-Sparen: Claude nur fuer vielversprechende Signale ──
      // Schlechte Signale (Score < 55) bekommen sofort regel-basierte Analyse
      // Spart 60-70% der Claude API Calls
      ...
      // ── Qualitätsfilter: nur wirklich gute Signale ──
      // R/R berechnen
      let rr = 0;
      ...
      // rr=0 means Claude didn't provide TP/SL prices yet - don't block on it
      ...
      // Log warum kein Telegram gesendet wurde
      if (!shouldSendTelegram && url.pathname === "/webhook" && ai.recommendation === "RECOMMENDED") {
        const reasons = [];
        if (ai.score < 70) reasons.push(`Score ${ai.score}<70`);
        if (ai.risk === "HIGH") reasons.push("Risiko HIGH");
        if (rr < 1.5) reasons.push(`R/R ${rr.toFixed(2)}<1.5`);
        if (!tfAligned) reasons.push("4H/1H nicht aligned");
        console.log(`Signal gefiltert (${signal.symbol}): ${reasons.join(", ")}`);
      }
      ...
      // 🔔 Extra Push bei EXCELLENT Signal (Score >= 85)
...
async function saveSnapshot(env, signal) {
  // Normalize timeframe: TradingView uses "5", "60", "240" but our Pine Script sends "5", "1H", "4H"
  let tf = signal.timeframe || "5";
  if (tf === "60") tf = "1H";
  if (tf === "240") tf = "4H";
  if (tf === "5m") tf = "5";
  signal.timeframe = tf;
  ...
  try {
    ...
    return (result.results || [])
      .filter(row => {
        ...
        // Show only 5min snapshots in main list (1H/4H are loaded separately for analysis)
        return tf === "5" || tf === "5m";
      })
      .map(row => {
...
function formatTelegram(signal, ai, ruleScore, priority) {
  ...
  const dirEmoji = dir === "LONG" ? "\u{1F7E2}" : dir === "SHORT" ? "\u{1F534}" : "\u26AA";
  const decEmoji = rec ? "\u2705" : "\u274C";
  const riskEmoji = ai.risk === "LOW" ? "\u{1F7E2}" : ai.risk === "MEDIUM" ? "\u{1F7E1}" : "\u{1F534}";
  ...
  let rrStr = "-";
  if (ai.entry && ai.take_profit && ai.stop_loss && ai.entry !== ai.stop_loss) {
    const rr = Math.abs(ai.take_profit - ai.entry) / Math.abs(ai.entry - ai.stop_loss);
    rrStr = "1:" + rr.toFixed(2);
  }

  const decisionText = rec
    ? (score >= 85 ? "EXCELLENT - TRADE DAS JETZT" : "TRADE DAS - Setup erfuellt")
    : "NICHT TRADEN - Setup unvollstaendig";

  const lines = [
    decEmoji + " *WAVESCOUT Signal*",
    "\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501",
    "*" + sym + "* " + dirEmoji + " " + dir,
    "",
    "Score: *" + score + "/100*",
    ...
    "R/R:    `" + rrStr + "`",
    "",
    "Risiko: " + riskEmoji + " " + (ai.risk || "?"),
    "Conf.:  " + (ai.confidence || 0) + "%",
    ai.reason ? "" : null,
    ai.reason ? "Analyse: " + ai.reason : null,
    "",
    "\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501",
    "WAVESCOUT v3 | " + (signal.timeframe || "5m")
...
async function dailySummary(env) {
  ...
  try {
    ...
    const rows = await env.DB.prepare(`
      SELECT outcome, ai_score, ai_direction, symbol, pnl_pct
      FROM signals
    ...
    if (trades.length === 0) {
      await sendTelegram(env, `📊 *WAVESCOUT Daily Summary*
_${yDate}_

Keine abgeschlossenen Trades gestern.`);
      return;
    }
    ...
    const totalPnl = trades.reduce((s, t) => s + (Number(t.pnl_pct) || 0), 0);
    const avgScore = trades.length > 0 ? Math.round(trades.reduce((s, t) => s + (Number(t.ai_score) || 0), 0) / trades.length) : 0;

    const bars = Math.round(winrate / 10);
    const bar = '█'.repeat(bars) + '░'.repeat(10 - bars);

    let msg = `📊 *WAVESCOUT Daily Summary*
`;
    msg += `_${yDate}_
`;
    msg += `━━━━━━━━━━━━━━━━━━
...
`;
    msg += `💰 P&L: *${totalPnl > 0 ? '+' : ''}${totalPnl.toFixed(2)}%*
`;
...
`;
      trades.slice(0, 8).forEach(t => {
        const dir = t.ai_direction === 'LONG' ? '🟢' : '🔴';
        const oc = t.outcome === 'WIN' ? '✅' : '❌';
        msg += `${oc} ${dir} ${t.symbol} (${Number(t.ai_score)||0}/100)
`;
      });
    }
...
// Simple wrapper using env chat ID
